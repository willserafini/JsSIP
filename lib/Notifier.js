...
module.exports = class Notifier extends EventEmitter 
{
    ...
    credential, pending }) 
  {
    ...
    // Set expires timerstamp and timer
  ...
  { 
    ...
    const sendFinalNotify = (reason === C.TERMINATED_RECEIVED_UNSUBSCRIBE 
      || reason === C.TERMINATED_SUBSCRIPTION_EXPIRED);
  
    debug(`emit "terminated" ${reason}, sendFinalNotify=${sendFinalNotify}`);
  ...
  _setExpiresTimer() 
  {
    ...
    setTimeout(() => 
    {
      ...
      this.is_final_notify_sent = true;
      this.sendNotify();
      this._dialogTerminated(C.TERMINATED_SUBSCRIPTION_EXPIRED);
    }, this.expires * 1000);
  }
};